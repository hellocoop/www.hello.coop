...
function extractDescriptionFromHtml(html) {
    ...
    const h1Match = body.match(
        /<h1[^>]*>([^<]+)<\/h1>[\s\S]{0,500}(?:<p[^>]*>([^<]+)<\/p>|<div[^>]*>([^<]{50,200})<\/div>)/i
    )
    if (h1Match) {
...
function extractMetadata(html, filePath) {
    ...
    if (relativePath === 'index.html') {
        ...
        // For Next.js static export, paths are like: approach/index.html -> /approach/
        ...
        if (dirPath === '.') {
            ...
            // Nested directory
            metadata.url = `/${dirPath}${fileName === 'index' ? '/' : `/${fileName}`}`
        }
    }

    // Check for markdown files in S3/markdown/ directory
    metadata.markdownUrl = null

    // Map HTML paths to markdown files in S3/markdown/ structure
    if (relativePath === 'index.html') {
        ...
        if (existsSync(markdownPath)) {
            ...
            if (
                ...
                (!metadata.description ||
                    metadata.description === 'Co-operatively Building the Internet Identity Layer')
            ) {
        ...
        // Check for markdown files in S3/markdown/ structure
        let markdownPath
        let markdownUrl

        if (dirPath === '.') {
            // Root level pages (like products)
            if (fileName === 'products') {
                markdownPath = 'S3/markdown/products.md'
                markdownUrl = '/markdown/products.md'
            }
        } else if (dirPath.startsWith('approach/')) {
            // Approach pages go to markdown/approach/
            // Extract page name from path like "approach/cooperative" -> "cooperative"
            const parts = dirPath.split('/')
            const pageName = parts[parts.length - 1] // Get last part
            markdownPath = `S3/markdown/approach/${pageName}.md`
            markdownUrl = `/markdown/approach/${pageName}.md`
        } else if (
            dirPath === 'privacy-policy' ||
            dirPath === 'terms-of-service' ||
            dirPath === 'trademark-disclaimer'
        ) {
            // Legal pages go to markdown/legal/
            markdownPath = `S3/markdown/legal/${dirPath}.md`
            markdownUrl = `/markdown/legal/${dirPath}.md`
        }

        if (markdownPath && existsSync(markdownPath)) {
            metadata.markdownUrl = markdownUrl
            // Try to get better description from markdown
            const mdDesc = extractDescriptionFromMarkdown(markdownPath)
            if (
                mdDesc &&
...
function getHtmlFiles(dir, base = '') {
    ...
    for (const file of files) {
        ...
        if (stat.isDirectory()) {
            // Skip node_modules, _next, and other build artifacts
            if (
                file === 'node_modules' ||
                file === '_next' ||
                file === 'dev' ||
...
function generateLlmsTxt() {
    ...
    if (!existsSync('S3')) {
        console.error('❌ S3 directory not found. Please run "npm run build" first.')
        process.exit(1)
    }

    // Process all HTML files in S3 directory
    const htmlFiles = getHtmlFiles('S3')
    ...
    content += `- **Products** (\`/products/\`): Detailed product information\n`
    content += `- **Pages** (\`/approach/\`): Documentation and informational pages (also available as markdown in \`/markdown/approach/\`)\n`
    content += `- **Legal** (\`/privacy-policy/\`, \`/terms-of-service/\`, etc.): Legal documents (also available as markdown in \`/markdown/legal/\`)\n\n`
    ...
    content += `- **Approach markdown:** \`/markdown/approach/\` - Source markdown for documentation pages\n`
    ...
    content += `- Blog: https://blog.hello.coop/\n\n`
