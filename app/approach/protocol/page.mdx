...
So far, we have provided each application the same identifier for the same user. To address this, we create a layer of abstraction with a Release Record that links the Hellō identifier with a new, [directed identifier](/approach/laws-of-identity#_4-directed-identity) specific to the application and user.
